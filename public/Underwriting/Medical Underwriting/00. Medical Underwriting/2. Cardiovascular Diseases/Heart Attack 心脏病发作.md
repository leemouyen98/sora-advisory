#Medical_Underwriting/Cardiovascular_Diseases

Happens when one or more areas of the heart muscle don’t get enough oxygen. This happens when blood flow to the heart muscle is blocked.

![[Heart Attack 01.jpg]]![[Heart Attack 02.jpg]]

![The Heart](https://youtu.be/T_t-0cAP1C4)
## EXAMPLE CASE OF HOSPITAL ADMISSION

> Heart Attack (acute myocardial infarction) + Angiogram + angioplasty (PTCA) + stenting in one of hospital on Jan 2020 (3 days admission) in Kuala Lumpur.
> - Total medical cost = RM58, 400 ++
>
> NOTE :
> Under normal circumstances without complication involved.
> All cost quoted is on estimated basis, it may not fully represent the actual cost.
> Sources from various medical bills 2015 & 2023.
> All information disclosed in this page serves as references only.

## FREQUENTLY ASKED QUESTIONS

> 1) Actual & date diagnosis ?
> 2) Any investigation tests such angiogram, blood & urine tests etc... ? If yes, please state the dates & results.
> 3) Any surgery has been performed ? If yes, please state the date & type of operation.
> 4) On medication / treatment ? If yes, what type & period of treatment.
> 5) Still on medical follow up ? If yes, how frequent (weekly / monthly etc) ?

## COMMON REQUIREMENTS

> 1) From client / doctor
> a) Any latest relevant reports / results such as blood / urine / angiogram ?
> b) Operation report if has been performed.

## EXAMPLE OF DISCLOSURE

> Diagnosed to have acute myocardiac infartion on dd/mm/yy by Dr xxx of yy Hospital. Discharged well on dd/mm/yy.
>
> Blood & urine test done on dd/mm/yy are attached here. Currently on medical follow up

## POSSIBILITIES OF UNDERWRITING OUTCOMES

> Life / TPD / CI / Medical insurance:
> - Not insurable