#Medical_Underwriting/Respiratory_Diseases

Is an infection of the main airways of the lungs (bronchi), causing them to become irritated and inflamed. The main airways branch off on either side of your windpipe (trachea). They lead to smaller and smaller airways inside your lungs called bronchioles.

![[Bronchitis 01.png]]

## EXAMPLE CASE OF HOSPITAL ADMISSION

> ACUTE BRONCHITIS WITH ACUTE SINUSITIS IN ONE OF PRIVATE HOSPITAL ON APRIL 2018 (5 DAYS ADMISSION) IN KLANG, SELANGOR
> — TOTAL MEDICAL COST = RM11,700 ++
> 
> NOTE :
> All cost quoted is on estimated basis, it may not fully represent the actual cost.  
> Sources from various medical bills 2015 & 2023.  
> All information disclosed in this page serves as references only.

## FREQUENTLY ASKED QUESTIONS

> 1) Actual & date diagnosis ?
> 2) Any investigation tests such lung function test, blood & urine tests etc... ? If yes, please state the dates & results.
> 3) On medication / treatment ? If yes, what type & period of treatment.
> 4) Still on medical follow up ? If yes, how frequent (weekly / monthly etc) ?
> 5) Fully recovered ?

## COMMON REQUIREMENTS

> 1) From client / doctor  
> a) Any relevant reports such as blood / urine / scan test ?
> 
> 2) Medical attendant report (MAR) / Attending physician statement (APS)

## EXAMPLE OF DISCLOSURE

> Diagnosed to have 1 episode of bronchitis on dd/mm/yy by Dr xxx of yy Hospital, completely well treated & no complication noted.
> Scanning, blood & urine test done on dd/mm/yy is attached. 
> Fully recovered since.

## POSSIBILITIES OF UNDERWRITING OUTCOMES

> Life / TPD / CI / Medical insurance:
> – Standard case if fully recovered